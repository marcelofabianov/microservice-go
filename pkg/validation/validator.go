...
func (vi *validatorImpl) Struct(ctx context.Context, s any) error {
	...
	if valErrs, ok := err.(validator.ValidationErrors); ok {
		...
		if vi.config.EnableLogging {
			vi.log.ErrorContext(ctx, "Struct validation failed",
	...
	if vi.config.EnableLogging {
		vi.log.ErrorContext(ctx, "Unexpected validation error",
			"struct_type", fmt.Sprintf("%T", s),
			"error", err.Error(),
		)
	}
...
func (vi *validatorImpl) Field(ctx context.Context, field any, tag string) error {
	...
	if valErrs, ok := err.(validator.ValidationErrors); ok {
		...
		if vi.config.EnableLogging {
			vi.log.ErrorContext(ctx, "Field validation failed",
	...
	if vi.config.EnableLogging {
		vi.log.ErrorContext(ctx, "Unexpected field validation error",
			"tag", tag,
			"error", err.Error(),
		)
	}
